...
<?php
    ...
    if($soluong>0) {
        echo '<form method = "post" action ="">';
                ...
                echo "<li class='nuoc'>";
                echo '<input type="text" name="name[]" style = "width: 200px;height: 40px;font-size: 20px;" placeholder = "Tên đối tượng">';
                echo "</li>";
    ...
    if(isset($_POST['name']) && isset($_POST['value'])) {
        ...
        echo '<table border="1" width="500" height="auto">';
            ...
            {
                {
                    $option = ($values[$i] / $total) * 100;
                    echo '<tr>';
                    echo '<td style="width: 50px;height: 40px;">';
                    echo $names[$i];
                    echo '</td>';
                    echo '<td>';
                    echo '<div style="border: 1px solid red;height: 15px;width: ' . $option . '%;background-color: red;float: left">';
                    echo '' . $option . '%';
                    echo '</div>';
                    echo '</td>';
                    echo '</tr>';
